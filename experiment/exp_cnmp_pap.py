...
def pap_predict_test(mp_net, exp_title, **kwargs):
    ...
    # Loss
    ll_loss = -result_dict["loss"]
    util.print_line_title("log likelihood: {}".format(ll_loss))
    ...
    ctx_value = original_encoder_input["ctx"]["value"].cpu().numpy().squeeze()

    # ground truth
    test_batch = result_dict["test_batch"]
    tar_true_values = test_batch["c_pos_2"]["value"].cpu().numpy().squeeze()
    tar_true_times = test_batch["c_pos_2"]["time"].cpu().numpy().squeeze()[0]

    for i in range(len(ctx_value)):
        plt.figure()
        # ground truth
        plt.plot(tar_true_times, tar_true_values[i], alpha=0.3)
        # predict
        plt.plot(tar_true_times, pred_mean[i])
        plt.fill_between(tar_true_times,
                         pred_mean[i] - 2 * pred_L[i],
                         pred_mean[i] + 2 * pred_L[i],
                         color='gray',
                         alpha=0.5,
                         zorder=1)
        # others
        bc_index = int(ctx_value[i, 0])
        plt.axvline(tar_true_times[bc_index], alpha=0.2)
        plt.plot(tar_true_times[-1], tar_true_values[i, -1], marker="*")
        plt.ylim([0.4, 1.0])
        plt.show()
...
def pap_predict(mp_net, dict_pred_ctx, **kwargs):
    ...
    dict_decoder_input = {'c_pos_2': {'time': time[None, time_re_ctx:, None]}}
    ...
    dict_result = {'traj': traj,
                   ...
                   'pred_mean': pred_mean,
                   'pred_std': pred_L,
                   'replanning_time_index': time_re_ctx}
    return dict_result
...
def cnmp_pap_exp(**kwargs):
    # What are my configs?
    util.print_wrap_title("CNMP of PickAndPlace")

    config_path = util.get_config_path("CNMP_PAP_config")  # NN config
    ...
    mp_net = MPNet(config,
                   ...
                   kwargs["model_api"])
    mp_net.fit()
    exp_title = "{}, Epoch: {}".format("CNMP_PAP", mp_net.epoch)
...
def main():
    test(exp='cnmp_pap', restart=True)
    # test(exp='cnmp_pap', restart=False)
